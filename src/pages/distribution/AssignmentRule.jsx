...
const initialEditRule = {
  ...
  conditions: [
    ...
    {
      field: "Customer Tier",
      operatorText: "IS ANY OF",
      value: "Enterprise, Platinum",
    },
  ],
  routeTo: "Team",
  target: "Global Ops Tier 3",
};
...
export default function AssignmentRule() {
  const [query, setQuery] = useState("");
  const [activeTab, setActiveTab] = useState("Active Rules");
  const [rules] = useState(initialRules);
  const [editRule, setEditRule] = useState(initialEditRule);
  ...
  const filteredRules = useMemo(() => {
    ...
    return rules.filter(
      (rule) =>
        ...
        rule.priority.toLowerCase().includes(q)
    );
  }, [query, rules]);
  ...
  const removeCondition = (idx) => {
    setEditRule((prev) => ({
      ...prev,
      conditions: prev.conditions.filter((_, index) => index !== idx),
    }));
  };

  return (
    <AppShell>
      <div className="-mx-8 -my-8 grid min-h-[calc(100vh-140px)] grid-cols-1 xl:grid-cols-[minmax(0,1fr)_560px]">
          ...
          <div className="space-y-8">
            ...
            <div className="border-b border-slate-200 dark:border-white/10">
              <div className="flex gap-10">
                {["Active Rules", "Drafts", "Archived"].map((tab) => (
                  <button
                    key={tab}
                  ...
                  >
                    {tab}
                    {tab === "Active Rules" ? " (12)" : tab === "Drafts" ? " (4)" : ""}
                  </button>
                ))}
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-white/10">
                    {filteredRules.map((rule) => (
                      ...
                      >
                        ...
                        <td className="px-7 py-7">
                          <span
                            className={`inline-flex rounded-xl px-4 py-2 text-sm font-black ${priorityClasses(
                              rule.priority
                            )}`}
                          >
                            {rule.priority}
          ...
          <div className="flex-1 space-y-10 overflow-y-auto px-8 py-8">
            ...
            <section className="space-y-6">
              ...
              <div className="space-y-4">
                {editRule.conditions.map((condition, idx) => (
                  <div
                  ...
                  {["Team", "Agent", "Round Robin"].map((item) => (
                    <label key={item} className="flex items-center gap-3">
                      <input
                        type="radio"
                        name="routeTo"
                        checked={editRule.routeTo === item}
                        onChange={() =>
                          setEditRule((prev) => ({ ...prev, routeTo: item }))
                        }
                        className="h-5 w-5"
                      />
                      <span className="text-2 font-bold text-slate-800 dark:text-white">
                        {item}
                      </span>
                    </label>
                  ))}
